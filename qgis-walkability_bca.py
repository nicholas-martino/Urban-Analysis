...
class Walkability(QgsProcessingAlgorithm):
    ...
    bca_fabric = 'bca_fabric'
    bca_buildings = 'bca_buildings'
...
Aggregation Scale = A network radius from each Sample feature thtat will define which BCA Fabric Layer features will be aggregate for the analysis;
...
BCA Fabric = A polygon layer based on the British Columbia Asssessment land fabric data;
Buildings = A polygon layer based on the British Columbia Asssessment building data;
...
max_view = Mean of the maximum view line of each intersection
...
pwm = Potential walkability measure conceptually based on indicators of Accessibility ((intersection density + visibility)/2), Density ((population density + job density)/2) and Diversity (Simpson's index of land use diversity) standardized and summed;                         
Nicholas Martino. nicholas.martino@hotmail.com''')
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        bca_fabric = self.parameterAsVectorLayer(parameters, self.bca_fabric, context)
        bca_buildings = self.parameterAsVectorLayer(parameters, self.bca_buildings, context)
        ...
        outputs['qgis:minimumboundinggeometry_1']=processing.run('qgis:minimumboundinggeometry',
               {'INPUT':outputs['qgis:serviceareafromlayer_2']['OUTPUT_LINES'],'OUTPUT':'memory:','TYPE':3}, context=context, feedback=feedback)
        outputs['native:extractbylocation_1']=processing.run('native:extractbylocation', 
               {'INPUT':sample_input,'INTERSECT':outputs['qgis:minimumboundinggeometry_1']['OUTPUT'],'PREDICATE':0,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:addautoincrementalfield_1']=processing.run('native:addautoincrementalfield', {'FIELD_NAME':"id_elements",'INPUT':outputs['native:extractbylocation_1']['OUTPUT'],'SORT_ASCendING':True,'FIELD_NAME':"id_elements",'SORT_NULLS_FIRST':False,'START':1,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        outputs['native:addautoincrementalfield_2']=processing.run('native:addautoincrementalfield', {'FIELD_NAME':"id_intersections", 'INPUT':outputs['native:extractbylocation_3']['OUTPUT'],'SORT_ASCendING':True,'SORT_NULLS_FIRST':False,'START':1,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:extractbylocation_2']=processing.run('native:extractbylocation', {'INPUT':bca_fabric,'INTERSECT':outputs['qgis:minimumboundinggeometry_2']['OUTPUT'],'PREDICATE':0,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        # Calculate viewshed lines
        outputs['native:extractbylocation_isov']=processing.run('native:extractbylocation', {'INPUT':bca_buildings,'INTERSECT':outputs['qgis:minimumboundinggeometry_2']['OUTPUT'],'PREDICATE':0,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:fixgeometries_3isov']=processing.run('native:fixgeometries', {'INPUT':outputs['native:extractbylocation_isov']['OUTPUT'],'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:convexhull_1isov']=processing.run('native:convexhull', {'INPUT':outputs['native:fixgeometries_3isov']['OUTPUT'],'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:simplifygeometries_1isov']=processing.run('native:simplifygeometries', {'INPUT':outputs['native:convexhull_1isov']['OUTPUT'],'METHOD':0,'TOLERANCE':2,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['qgis:polygonstolines_1isov']=processing.run('qgis:polygonstolines', {'INPUT':outputs['native:simplifygeometries_1isov']['OUTPUT'],'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:offsetline_1isov']=processing.run('native:offsetline', {'DISTANCE':2,'INPUT':outputs['qgis:polygonstolines_1isov']['OUTPUT'],'JOIN_STYLE':2,'MITER_LIMIT':2,'SEGMENTS':8,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:extractvertices_1isov']=processing.run('native:extractvertices', {'INPUT':outputs['native:offsetline_1isov']['OUTPUT'],'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:addautoincrementalfield_origin']=processing.run('native:addautoincrementalfield', {'FIELD_NAME':"spoke_id",'INPUT':outputs['native:extractvertices_1isov']['OUTPUT'],'SORT_ASCENDING':True,'SORT_NULLS_FIRST':False,'START':0,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['native:addautoincrementalfield_target']=processing.run('native:addautoincrementalfield', {'FIELD_NAME':"hub_id",'INPUT':outputs['native:centroids_2']['OUTPUT'],'SORT_ASCENDING':True,'SORT_NULLS_FIRST':False,'START':0,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['qgis:distancematrix_1isov']=processing.run('qgis:distancematrix', {'INPUT':outputs['native:addautoincrementalfield_origin']['OUTPUT'],'INPUT_FIELD':"spoke_id",'MATRIX_TYPE':0,'NEAREST_POINTS':5,'TARGET':outputs['native:addautoincrementalfield_target']['OUTPUT'],'TARGET_FIELD':"hub_id",'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['qgis:fieldcalculator_1isov']=processing.run('qgis:fieldcalculator', {'FIELD_LENGTH':10,'FIELD_NAME':"id_aggregate",'FIELD_PRECISION':3,'FIELD_TYPE':2,'FORMULA':"TargetID",'INPUT':outputs['qgis:distancematrix_1isov']['OUTPUT'],'NEW_FIELD':True,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        aggregate1 = [{'input':'InputID','aggregate':'mean','delimiter':',','name':'input_id','type':6,'length':10,'precision':2},
                      {'input':'TargetID','aggregate':'mean','delimiter':',','name':'id_intersections','type':6,'length':10,'precision':2},
                      {'input':'Distance','aggregate':'max','delimiter':',','name':'max_view','type':6,'length':10,'precision':0}]   
        outputs['qgis:aggregate_1isov']=processing.run('qgis:aggregate', {'GROUP_BY':"id_aggregate",'INPUT':outputs['qgis:fieldcalculator_1isov']['OUTPUT'],'AGGREGATES':aggregate1,'OUTPUT':'memory:'}, context=context, feedback=feedback)       
        ...
        outputs['qgis:snapgeometries_2']=processing.run('qgis:snapgeometries', {'BEHAVIOR':3,'INPUT':outputs['qgis:aggregate_1isov']['OUTPUT'],'REFERENCE_LAYER':street_network,'TOLERANCE':100,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        outputs['qgis:joinbylocationsummary_2']=processing.run('qgis:joinbylocationsummary', {'DISCARD_NONMATCHING':False,'INPUT':outputs['qgis:fieldcalculator_2']['OUTPUT'],'JOIN':outputs['native:buffer_3']['OUTPUT'],'JOIN_FIELDS':"id_intersections;max_view",'PREDICATE':0,'SUMMARIES':[0,6],'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        outputs['native:joinattributestable_2']=processing.run('native:joinattributestable', {'DISCARD_NONMATCHING':False,'FIELD':"id_elements",'FIELDS_TO_COPY':"id_intersections_count;max_view_mean",'FIELD_2':"id_elements",'INPUT':outputs['qgis:joinbylocationsummary_3']['OUTPUT'],'INPUT_2':outputs['qgis:joinbylocationsummary_2']['OUTPUT'],'METHOD':1,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        outputs['qgis:fieldcalculator_27']=processing.run('qgis:fieldcalculator', {'FIELD_LENGTH':10,'FIELD_NAME':"retail_far",'FIELD_PRECISION':4,'FIELD_TYPE':0,'FORMULA':'''("ret_sum")/("ret_sum"+"off_sum"+"ent_sum")''','INPUT':outputs['qgis:fieldcalculator_7']['OUTPUT'],'NEW_FIELD':True,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        * (ln(("ent_sum")/("res_sum"+"ret_sum"+"off_sum"+"ins_sum"+"ent_sum"))))
        ...
        'INPUT':outputs['qgis:fieldcalculator_27']['OUTPUT'],'NEW_FIELD':True,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        'FORMULA':'''1-((("res_sum"*( "res_sum" -1))+("ret_sum"*( "ret_sum" -1))+("off_sum"*( "off_sum" -1))+("ins_sum"*( "ins_sum" -1))+("ent_sum"*( "ent_sum" -1)))/(("area_sum")*("area_sum"-1)))''',
        ...
        outputs['qgis:fieldcalculator_14']=processing.run('qgis:fieldcalculator', {'FIELD_LENGTH':10,'FIELD_NAME':"z_use_diversity",'FIELD_PRECISION':3,'FIELD_TYPE':0,
                'FORMULA':'''(("land_use_mix"-mean("land_use_mix"))/stdev("land_use_mix"))''',
                'INPUT':outputs['qgis:fieldcalculator_13']['OUTPUT'],'NEW_FIELD':True,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        outputs['qgis:fieldcalculator_22']=processing.run('qgis:fieldcalculator', {'FIELD_LENGTH':10,'FIELD_NAME':"z_number_of_intersections",'FIELD_PRECISION':3,'FIELD_TYPE':0,
               ...
               'INPUT':outputs['qgis:fieldcalculator_16']['OUTPUT'],'NEW_FIELD':True,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        ...
        outputs['qgis:fieldcalculator_21']=processing.run('qgis:fieldcalculator', {
                'FIELD_LENGTH':10,'FIELD_NAME':"z_retail_far",'FIELD_PRECISION':3,'FIELD_TYPE':0,
                'FORMULA':'''(("retail_far"-mean("retail_far"))/stdev("retail_far"))''',
                'INPUT':outputs['qgis:fieldcalculator_20']['OUTPUT'],'NEW_FIELD':True,'OUTPUT':'memory:'}, context=context, feedback=feedback)
        outputs['qgis:fieldcalculator_24']=processing.run('qgis:fieldcalculator', 
               {'FIELD_LENGTH':10,'FIELD_NAME':"z_use_diversity_simpson",'FIELD_PRECISION':4,'FIELD_TYPE':0,
        ...
        outputs['z_max_view']=processing.run('qgis:fieldcalculator', 
               {'FIELD_LENGTH':10,'FIELD_NAME':"z_max_view",'FIELD_PRECISION':4,'FIELD_TYPE':0,
                'FORMULA':'''(("max_view_mean"-mean("max_view_mean"))/stdev("max_view_mean"))''',
                'INPUT':outputs['qgis:fieldcalculator_24']['OUTPUT'],'NEW_FIELD':True,'OUTPUT':'memory:'}, context=context, feedback=feedback)

        # Calculate indexes
        outputs['qgis:fieldcalculator_19']=processing.run('qgis:fieldcalculator', 
               {'FIELD_LENGTH':10,'FIELD_NAME':"lfw",'FIELD_PRECISION':3,'FIELD_TYPE':0,
